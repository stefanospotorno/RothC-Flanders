...
#Df_BAU<-data.frame()
Df_BAU<-matrix(as.double(NA), dim(Variables)[1]*Years_of_simulation, 5)
Df_BAU_min<-matrix(as.double(NA), dim(Variables)[1]*Years_of_simulation, 5)
Df_BAU_max<-matrix(as.double(NA), dim(Variables)[1]*Years_of_simulation, 5)
  ...
  if (any(is.na(Evp[,2])) | any(is.na(Temp[,2])) | any(is.na(SOC_im[i])) | any(is.na(clay_im[i])) | any(is.na(Precip[,2]))  |  any(is.na(Cov2[,2]))  |  any(is.na(Cov1[,1])) | any(is.na(Cinputs_im[i])) | any(is.na(DR_im[i])) | (Cinputs_im[i]<0) |  (SOC_im[i]<0) | (clay_im[i]<0) ) {FORWARD[i,2]<-0}else{
    ...
    for (w in 1:Years_of_simulation) {
      if (w==1) {
        ...
        f_wp<-round(f_wp, 1)
        # Df_BAU<-rbind(Df_BAU,f_wp)
        Df_BAU[Years_of_simulation*(i-1)+w,]<-f_wp
