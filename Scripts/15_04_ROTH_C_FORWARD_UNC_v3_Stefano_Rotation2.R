...
# DR_im<-Variables[[40]]
DR_im<- rep(1.32,length(Variables[[40]])) #Decomposition rate (DPM/RPM) average between crop specific for silage maize and yellow mustard
...
Df_rot2<-matrix(as.double(NA), dim(Variables)[1]*Years_of_simulation, 5)
...
f_rot2<-Roth_C(Cinputs=(Cinput_2*CF_NPP),years=years1,DPMptf=WARM_UP@data[[i,5]], RPMptf=WARM_UP@data[[i,6]], BIOptf=WARM_UP@data[[i,7]], HUMptf=WARM_UP@data[[i,8]], FallIOM=WARM_UP@data[[i,9]],Temp=Temp,Precip=Precip,Evp=Evp,Cov=Cov,Cov1=Cov1,Cov2=Cov2,soil.thick=soil.thick,SOC=SOC,clay=clay,DR=DR,bare1=bare1,LU=LU)
f_rot2_t<-f_rot2[1]+f_rot2[2]+f_rot2[3]+f_rot2[4]+f_rot2[5]
f_rot2<-round(f_rot2, 1)
Df_rot2[Years_of_simulation*(i-1)+w,]<-f_rot2
} else {
  f_rot2<-Roth_C(Cinputs=(Cinput_2*CF_NPP),years=years1,DPMptf=f_rot2[1], RPMptf=f_rot2[2], BIOptf=f_rot2[3], HUMptf=f_rot2[4], FallIOM=f_rot2[5],Temp=Temp,Precip=Precip,Evp=Evp,Cov=Cov,Cov1=Cov1,Cov2=Cov2,soil.thick=soil.thick,SOC=SOC,clay=clay,DR=DR,bare1=bare1,LU=LU)
  f_rot2<-round(f_rot2, 1)

  # Df_rot2<-rbind(Df_rot2,f_rot2)
  Df_rot2[Years_of_simulation*(i-1)+w,]<-f_rot2
}
  f_rot2_t<-f_rot2[1]+f_rot2[2]+f_rot2[3]+f_rot2[4]+f_rot2[5]
...
Df_rot2<-as.data.frame(Df_rot2)

colnames(FORWARD@data)[2]="SOC_t0"
colnames(FORWARD@data)[3]="SOC_rot2_20"
...
# Save the shapefile
shapefile(FORWARD, filename = file.path(output_dir, "FORWARD_04_AL_Fl_rot2_V1.shp"),overwrite=TRUE)


########## ROTATION 1 #######################
# Rename Df: Df stores SOC-pool at end of each year

colnames(Df_rot2) <- c("DPM", "RPM", "BIO", "HUM", "IOM")
SOC_total <- Df_rot2$DPM + Df_rot2$RPM + Df_rot2$BIO + Df_rot2$HUM + Df_rot2$IOM
Df_rot2<-cbind(Df_rot2, SOC_total)
colnames(Df_rot2) <- c("DPM", "RPM", "BIO", "HUM", "IOM","SOC_rot2")
ID_gap<-Years_of_simulation
Df_rot2$ID<-rep(seq(1, 1+nrow(Df_rot2)%/% ID_gap), each = ID_gap, length.out = nrow(Df_rot2))
Df_rot2$year<- rep(seq(1:Years_of_simulation))
Df_rot2$SOC_rot2_max<- (Df_rot2$SOC_rot2*runif(1, min = 1.01, max = 1.04))
Df_rot2$SOC_rot2_min<- (Df_rot2$SOC_rot2*runif(1, min = 0.96, max = 0.99))

##############################################


Df_rot2<- Df_rot2[,-c(1:5)]
Df_rot2<-Df_rot2[,-2]

colnames(Df_rot2)=c("SOC_rot2","year",  "SOC_rot2_max", "SOC_rot2_min")


SOC_graph_rot2<-as.data.frame(Df_rot2)
graph_years_rot2<-aggregate(. ~ year, SOC_graph_rot2, median)
...
colnames(yearzero)=c("SOC_rot2","year",  "SOC_rot2_max", "SOC_rot2_min")
# graph_years_rot2<-rbind(graph_years_rot2,yearzero)
#######################################
...
ggplot(graph_years_rot2, aes(x = year, y = SOC_rot2)) +
  geom_line() +
  ...
  geom_pointrange(aes(ymin=SOC_rot2_min, ymax=SOC_rot2_max)) +
...
save(Df_rot2,file="SOC_rot2_AL_V1.Rdata")
save(graph_years_rot2,file="SOC_Graph_rot2_AL_V1.Rdata")
