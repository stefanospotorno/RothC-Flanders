...
Vector<-shapefile("TargetPoints/AgriculturalpointsFromR.shp")


# Stack_Set_1 is a stack that contains the spatial variables 
Stack_Set_1<- stack("Stacks_Harmonization/Stack_Set_SPIN_UP_AL_Fl.tif")
...
Vector_variables<-raster::extract(Stack_Set_1,Vector,df=TRUE)

# Extract the layers from the Vector

SOC_im<-Vector_variables[[2]] # first stack band

clay_im<-Vector_variables[[3]] # second stack band
...
for (i in 1:dim(Vector_variables)[1]) {
  ...
  ##############################################################################  
    ...
    # SOC POOLS AFTER 10000 YEARS RUN WITH C INPUT EQUILIBRIUM
...
# shapefile(C_INPUT_EQ, ".", "SPIN_UP_04_AL_Fl_V1", driver="ESRI Shapefile",overwrite=TRUE) 
