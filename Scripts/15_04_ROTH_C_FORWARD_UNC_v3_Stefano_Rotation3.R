...
# DR_im<-Variables[[40]]
DR_im<- rep(1.335,length(Variables[[40]])) #Decomposition rate (DPM/RPM) average between crop specific for silage maize, winter cereals and yellow mustard
...
# 3) 1 x silage maize - 2 x winter wheat
...
Df_rot3<-matrix(as.double(NA), dim(Variables)[1]*Years_of_simulation, 5)
...
f_rot3<-Roth_C(Cinputs=(Cinput_3*CF_NPP),years=years1,DPMptf=WARM_UP@data[[i,5]], RPMptf=WARM_UP@data[[i,6]], BIOptf=WARM_UP@data[[i,7]], HUMptf=WARM_UP@data[[i,8]], FallIOM=WARM_UP@data[[i,9]],Temp=Temp,Precip=Precip,Evp=Evp,Cov=Cov,Cov1=Cov1,Cov2=Cov2,soil.thick=soil.thick,SOC=SOC,clay=clay,DR=DR,bare1=bare1,LU=LU)
f_rot3_t<-f_rot3[1]+f_rot3[2]+f_rot3[3]+f_rot3[4]+f_rot3[5]
f_rot3<-round(f_rot3, 1)
Df_rot3[Years_of_simulation*(i-1)+w,]<-f_rot3
} else {
  f_rot3<-Roth_C(Cinputs=(Cinput_3*CF_NPP),years=years1,DPMptf=f_rot3[1], RPMptf=f_rot3[2], BIOptf=f_rot3[3], HUMptf=f_rot3[4], FallIOM=f_rot3[5],Temp=Temp,Precip=Precip,Evp=Evp,Cov=Cov,Cov1=Cov1,Cov2=Cov2,soil.thick=soil.thick,SOC=SOC,clay=clay,DR=DR,bare1=bare1,LU=LU)
  f_rot3<-round(f_rot3, 1)

  # Df_rot3<-rbind(Df_rot3,f_rot3)
  Df_rot3[Years_of_simulation*(i-1)+w,]<-f_rot3
}
  f_rot3_t<-f_rot3[1]+f_rot3[2]+f_rot3[3]+f_rot3[4]+f_rot3[5]
...
Df_rot3<-as.data.frame(Df_rot3)


colnames(FORWARD@data)[2]="SOC_t0"
colnames(FORWARD@data)[3]="SOC_rot3_20"
...
# Save the shapefile
shapefile(FORWARD, filename = file.path(output_dir, "FORWARD_04_AL_Fl_rot3_V1.shp"),overwrite=TRUE)


########## ROTATION 1 #######################
# Rename Df: Df stores SOC-pool at end of each year

colnames(Df_rot3) <- c("DPM", "RPM", "BIO", "HUM", "IOM")
SOC_total <- Df_rot3$DPM + Df_rot3$RPM + Df_rot3$BIO + Df_rot3$HUM + Df_rot3$IOM
Df_rot3<-cbind(Df_rot3, SOC_total)
colnames(Df_rot3) <- c("DPM", "RPM", "BIO", "HUM", "IOM","SOC_rot3")
ID_gap<-Years_of_simulation
Df_rot3$ID<-rep(seq(1, 1+nrow(Df_rot3)%/% ID_gap), each = ID_gap, length.out = nrow(Df_rot3))
Df_rot3$year<- rep(seq(1:Years_of_simulation))
Df_rot3$SOC_rot3_max<- (Df_rot3$SOC_rot3*runif(1, min = 1.01, max = 1.04))
Df_rot3$SOC_rot3_min<- (Df_rot3$SOC_rot3*runif(1, min = 0.96, max = 0.99))

##############################################


Df_rot3<- Df_rot3[,-c(1:5)]
Df_rot3<-Df_rot3[,-2]

colnames(Df_rot3)=c("SOC_rot3","year",  "SOC_rot3_max", "SOC_rot3_min")


SOC_graph_rot3<-as.data.frame(Df_rot3)
graph_years_rot3<-aggregate(. ~ year, SOC_graph_rot3, median)
yearzero<-c( median(WARM_UP$SOC_t0),0, median(WARM_UP$SOC_t0)*runif(1, min = 1.01, max = 1.04),median(WARM_UP$SOC_t0)*runif(1, min = 0.93, max = 0.99))
yearzero<-(as.data.frame(t(yearzero)))
colnames(yearzero)=c("SOC_rot3","year",  "SOC_rot3_max", "SOC_rot3_min")
# graph_years_rot3<-rbind(graph_years_rot3,yearzero)
#######################################
...
ggplot(graph_years_rot3, aes(x = year, y = SOC_rot3)) +
  geom_line() +
  ...
  geom_pointrange(aes(ymin=SOC_rot3_min, ymax=SOC_rot3_max)) +
...
save(Df_rot3,file="SOC_rot3_AL_V1.Rdata")
save(graph_years_rot3,file="SOC_Graph_rot3_AL_V1.Rdata")
