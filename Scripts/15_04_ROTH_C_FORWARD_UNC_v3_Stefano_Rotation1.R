...
Df_rot1<-matrix(as.double(NA), dim(Variables)[1]*Years_of_simulation, 5)
...
f_rot1<-Roth_C(Cinputs=(Cinput_1*CF_NPP),years=years1,DPMptf=WARM_UP@data[[i,5]], RPMptf=WARM_UP@data[[i,6]], BIOptf=WARM_UP@data[[i,7]], HUMptf=WARM_UP@data[[i,8]], FallIOM=WARM_UP@data[[i,9]],Temp=Temp,Precip=Precip,Evp=Evp,Cov=Cov,Cov1=Cov1,Cov2=Cov2,soil.thick=soil.thick,SOC=SOC,clay=clay,DR=DR,bare1=bare1,LU=LU)
f_rot1_t<-f_rot1[1]+f_rot1[2]+f_rot1[3]+f_rot1[4]+f_rot1[5]
f_rot1<-round(f_rot1, 1)
Df_rot1[Years_of_simulation*(i-1)+w,]<-f_rot1
} else {
  f_rot1<-Roth_C(Cinputs=(Cinput_1*CF_NPP),years=years1,DPMptf=f_rot1[1], RPMptf=f_rot1[2], BIOptf=f_rot1[3], HUMptf=f_rot1[4], FallIOM=f_rot1[5],Temp=Temp,Precip=Precip,Evp=Evp,Cov=Cov,Cov1=Cov1,Cov2=Cov2,soil.thick=soil.thick,SOC=SOC,clay=clay,DR=DR,bare1=bare1,LU=LU)
  f_rot1<-round(f_rot1, 1)
...
Df_rot1<-as.data.frame(Df_rot1)


colnames(FORWARD@data)[2]="SOC_t0"
colnames(FORWARD@data)[3]="SOC_rot1_20"
...
# Save the shapefile
shapefile(FORWARD, filename = file.path(output_dir, "FORWARD_04_AL_Fl_rot1_V1.shp"),overwrite=TRUE)

########## ROTATION 1 #######################
# Rename Df: Df stores SOC-pool at end of each year

colnames(Df_rot1) <- c("DPM", "RPM", "BIO", "HUM", "IOM")
SOC_total <- Df_rot1$DPM + Df_rot1$RPM + Df_rot1$BIO + Df_rot1$HUM + Df_rot1$IOM
Df_rot1<-cbind(Df_rot1, SOC_total)
colnames(Df_rot1) <- c("DPM", "RPM", "BIO", "HUM", "IOM","SOC_rot1")
ID_gap<-Years_of_simulation
Df_rot1$ID<-rep(seq(1, 1+nrow(Df_rot1)%/% ID_gap), each = ID_gap, length.out = nrow(Df_rot1))
Df_rot1$year<- rep(seq(1:Years_of_simulation))
Df_rot1$SOC_rot1_max<- (Df_rot1$SOC_rot1*runif(1, min = 1.01, max = 1.04))
Df_rot1$SOC_rot1_min<- (Df_rot1$SOC_rot1*runif(1, min = 0.96, max = 0.99))

##############################################


Df_rot1<- Df_rot1[,-c(1:5)]
Df_rot1<-Df_rot1[,-2]

colnames(Df_rot1)=c("SOC_rot1","year",  "SOC_rot1_max", "SOC_rot1_min")


SOC_graph_rot1<-as.data.frame(Df_rot1)
graph_years_rot1<-aggregate(. ~ year, SOC_graph_rot1, median)
...
colnames(yearzero)=c("SOC_rot1","year",  "SOC_rot1_max", "SOC_rot1_min")
# graph_years_rot1<-rbind(graph_years_rot1,yearzero)
#######################################
...
ggplot(graph_years_rot1, aes(x = year, y = SOC_rot1)) +
  geom_line() +
  ...
  geom_pointrange(aes(ymin=SOC_rot1_min, ymax=SOC_rot1_max)) +
...
save(Df_rot1,file="SOC_rot1_AL_V1.Rdata")
save(graph_years_rot1,file="SOC_Graph_rot1_AL_V1.Rdata")

# save(Df_rot1,file="SOC_rot1_AL_NPPnotTouched.Rdata")
# save(graph_years_rot1,file="SOC_Graph_rot1_AL_NPPnotTouched.Rdata")
